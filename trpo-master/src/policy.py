...
class Policy(object):
    ...
    def __init__(self, obs_dim, act_dim, kl_targ, hid1_mult, policy_logvar, clipping_range=None, filename = 'filename'):
        """
        Args:
        ...
        """
        self.filename = filename + 'logs_policy/'
    ...
    def _init_session(self):
        ...
        file_writer = tf.summary.FileWriter(self.filename, self.g)  # TensorBoardに対応
    ...
    def save(self, episode, filename):
        self.saver.save(self.sess, filename + str(episode))# + '.ckpt')
    def restore(self, count, filename):
        self.saver.restore(self.sess, filename + str(count))# + '.ckpt')
